...
studios <- readLines("bottom10_studios.txt")
...
for (stu in studios) {
  ...
  for (i in 1:len) {
    ...
    dir.create(paste0("bottom10studios_posters/", studio), showWarnings = FALSE)
    ggsave(
      filename = paste0("bottom10studios_posters/", studio, "/", rank, "_", gsub("[^a-zA-Z0-9]", "_", mv_name), ".png"),
      plot = combined,
      width = 10,
      height = 6,
      dpi = 300
